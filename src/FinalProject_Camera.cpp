...
int main(int argc, const char *argv[])
{
	...
	double totalMeanError = 0;
  	int nConsideredImages = 0;
    for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex; imgIndex+=imgStepWidth)
    {
      	double meanError = 0; int N = 0;
        ...
        if(bVis)
        {
            namedWindow("3D Objects", cv::WINDOW_NORMAL);
        ...
        if (detectorType.compare("SHITOMASI") == 0) {
            ...
            std::cerr << "Wrong detector type: " << detectorType << std::endl;
            std::cerr << "=> Valid detector types: SHITOMASI, HARRIS, FAST, "
        ...
        if (dataBuffer.size() > 1) // wait until at least two images have been processed
        {
            ...
            for (auto it1 = (dataBuffer.end() - 1)->bbMatches.begin(); it1 != (dataBuffer.end() - 1)->bbMatches.end(); ++it1)
            {
                ...
                if( currBB->lidarPoints.size()>0 && prevBB->lidarPoints.size()>0 ) // only compute TTC if we have Lidar points
                {
                    ...
                    double error = abs(ttcLidar - ttcCamera);
  	...
  	totalMeanError /= nConsideredImages;
  	cout << totalMeanError << "|" << endl;
